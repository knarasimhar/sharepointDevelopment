...
namespace BulkPushConsoleApp
{
    class Program
    {
        ...
        // GET api/values/5
        static String strSiteURL = "http://dspsvr/sites/Sharepoint2/testlocal", strUSER = "spuser2", strPWD = "User@123#", strADUserURL = "";
        ...
        static SPList Cache_workflow_History;
        static SPList Cache_pipflow;
        ...
        static private void spsetFMRDBBulk(string fmrid, string remarks, string Listname, string AssignedTo = "", string FY = "", string stateid = "", string fmrtype = "", string roleid = "")
        {
            ...
            SPList sourceList = web.Lists["pipflow1"];
          
            SPListItem newItem = sourceList.AddItem();
           ...
           // oListItem["Title"] = fmrid;
            ...
            if (AssignedTo != "")
            {
                SPUser newUser = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(AssignedTo));
                web.AllowUnsafeUpdates = true;

                //uAssignedTo = user.EnsureUser(strDomainName + HttpUtility.UrlDecode(AssignedTo));
                newItem["ry3a"] = newUser;
                newItem["currentAssignee"] = newUser;
            }
            newItem["roleid"] = roleid;
            newItem["FY"] = FY;
            newItem["stateid"] = stateid;
            if (fmrtype != null && fmrtype != "")
                newItem["fmrtype"] = fmrtype;
            ...
            string relatedItem = newItem.ID.ToString();
            ...
            SPList workflowhistory = web.Lists["workflow_history"];
            SPListItem whnewItem = workflowhistory.AddItem();
            //itemCreateInfo = new ListItemCreationInformation();
            // oListItem = oList.AddItem(itemCreateInfo);
            whnewItem["Title"] = fmrid;
            whnewItem["comments"] = "FMR to ADD Task";
            //oListItem["approveduser"] = _list2History["approveduser"];
            // oListItem["areviewuser"] = _list2History["areviewuser"];
            if (AssignedTo != "")
            {
                SPUser uAssignedTo = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(AssignedTo));
                whnewItem["Assigned_x0020_To"] = uAssignedTo;

            }
            // oListItem["Assigned_x0020_To"] = uAssignedTo;
            whnewItem["roleid"] = roleid;
            whnewItem["stateid"] = stateid;
            // oListItem["event"] = _list2History["event"];
            whnewItem["relateditem"] = relatedItem;
            whnewItem["tasktype"] = "1";
            whnewItem["TaskOutcome"] = "";

            whnewItem["Status"] = "Not Started";
        ...
        static private string spsetTaskItemByID_New(string status, string percentComplete, string Comments, string createdby, string taskid, ref ClientContext UpdateFMRclientContext, string assignevent = "", string AssignedTo = "", string areviewuserTo = "", string SPFmrID = "", string TASKTYPE = "", string stateid = "", string roleid = "")
        {
            // prepare site connection
            ...
            try
            {
                ...
                SPUser createuser = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(createdby));
                SPUser assignuser = null;
                ...
                {
                    Cache_workflow_History = oList = web.Lists["workflow_history"];
               ...
               // SPListItem whnewItem = workflowhistory.AddItem();
                ...
                SPListItem list2 = oList.GetItemById(Int32.Parse(taskid));
                ...
                */
                ...
                //oList = clientContext.Web.Lists.GetByTitle(cWfHListName);
                oList=web.Lists["workflow_history"];
                SPListItem newItem = oList.AddItem();
               // ListItemCreationInformation itemCreateInfo = new ListItemCreationInformation();
                ...
                if (AssignedTo != "")
                {
                    ...
                    {
                        assignuser = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(auser));
                ...
                if (areviewuserTo != null && areviewuserTo != "" && (TASKTYPE == "2" || TASKTYPE == "3"))
                {
                    ...
                    foreach (string auser in areviewuserTo.Split(','))
                    {
                        ...
                        SPListItem newItem2 = oList.AddItem();
                        assignuser = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(auser));
                        ...
                        newItem2["Assigned_x0020_To"] = createuser;
                        // below column is userd to set the aditional review user store
                        newItem2["areviewuser"] = AreveiweruserValueCollection;
                        if (TASKTYPE == "2")
                            newItem2["Title"] = "Additional Review";
                        else if (TASKTYPE == "3")
                            newItem2["Title"] = "ROP";
                        else
                            newItem2["Title"] = "sub task";
                        // oItem["ParentID"] = lookupValueCollection; // set chidl item ParentID field  
                        newItem2["tasktype"] = TASKTYPE;
                        newItem2["relateditem"] = SPFmrID;
                        newItem2["Status"] = status;
                        newItem2["TaskOutcome"] = "";
                        newItem2["roleid"] = roleid;
                        newItem2["stateid"] = stateid;
                        //  oItem["PercentComplete"] = 0;
                        newItem2.Update();
                        //clientContext.Load(newItem2);
                        // clientContext.ExecuteQuery();
        ...
        static private void setPreviousTaskHistory(ref SPListItem _list2History, string SPFmrID, string taskid, string Status, ref ClientContext clientContext)
        {
            // ClientContext clientContext = new ClientContext(strSiteURL);
            // clientContext.Credentials = new NetworkCredential("spm", "pip@123");
            SPSite site = new SPSite(strSiteURL);
            SPWeb web = site.OpenWeb();
            SPList whList = web.Lists["workflow_history"];

            SPListItem oListItem = whList.AddItem();
        ...
        static private string spsetAddorupdteItemByID(string status, string Listname, string Comments, string createdby, string itemid, string keyvalue, string Taskid, ref ClientContext clientContext, string stateid, string roleid, string AssignTo = "", string CurAssignTo = "")
        {
            ...
            try
            {
                ...
                {
                    Cache_pipflow = oList = web.Lists[Listname];//= clientContext.Web.Lists.GetByTitle(Listname);

                }

                SPListItem list2 = oList.GetItemById(Int32.Parse(itemid));
                ...
                */
                ...
                if (AssignTo != "")
                {
                    ...
                    {
                        SPUser uAssignedTo = web.EnsureUser(strDomainName + HttpUtility.UrlDecode(AssignTo));
                        //clientContext.Load(uAssignedTo);
                        //clientContext.ExecuteQuery();
                        _userstable.Add(AssignTo, uAssignedTo.ID);
                    }
        ...
        static async Task spgetListItemByID()
        {
            ...
            try
            {
                ...
                SPSite site = new SPSite(strSiteURL);
                SPWeb web = site.OpenWeb();
                //SPList sourceList = web.Lists[getConfigvalue("sourceListname")];
                ...
                query.Query = "<View><RowLimit>5000</RowLimit><Query><Where><Eq><FieldRef Name='stateid' /><Value Type='Number'>" + stateid + "</Value></Eq></Where></Query></View>";
                query.RowLimit = 2000;
                ...
                /* camlQuery.ViewXml = "<View><Query><Where><Or>" +
                                      ...
                                      "</Or></Where></Query></View>";*/
                //camlQuery.ViewXml = "<View><Query><Where><Eq><FieldRef Name='stateid' /><Value Type='Number'>" + stateid + "</Value></Eq></Where></Query></View>";
                ...
                SPList list = web.Lists["bulkpushapis"];
                
                SPListItemCollection olists = list.GetItems(query);
                ...
                foreach (BulkpushAPIS resp in respmsg)
                {
                    ...
                    if (resp.status == FMRStatus)
                    {
...

                        {
                            ...
                            var query1 = HttpUtility.ParseQueryString(uri.Query);
                            if (resp.url.ToLower().Contains("/spsetfmr?"))
                            {
                                QueryParams = JArray.Parse(ClsGeneral.GetJsonStringFromQueryString(query1.ToString().ToLower()));
                                ...
                                SPListItem targetListItem = list.GetItemById(Convert.ToInt32(resp.Id));
                    ...
                    else if (resp.status == wfhstatus)
                    {
...

                        {
                            ...
                            var uri = new Uri(resp.url);
                            var query2 = HttpUtility.ParseQueryString(uri.Query);
                            if (resp.url.ToLower().Contains("/spsettaskitembyid?"))
                            {
                                QueryParams = JArray.Parse(ClsGeneral.GetJsonStringFromQueryString(query2.ToString().ToLower()));
                                ...
                                try
                                {
                                    ...
                                    Stopwatch stopwatch = new Stopwatch();
                                    stopwatch.Start();
                                    spsetTaskItemByID_New(QueryParam.status.Value, QueryParam.percentcomplete.Value, QueryParam.comments.Value, QueryParam.createdby.Value, QueryParam.taskid.Value, ref UPdateFMRBulkclientContext, QueryParam.assignevent.Value, QueryParam.assignedto.Value, QueryParam.areviewuserto.Value, QueryParam.spfmrid.Value, QueryParam.tasktype.Value, stateid, roleid);
                                    stopwatch.Stop();
                    ...
                    else if (resp.status == CallbackStatus)
                    {
...

                        {
                            ...
                            if (resp.url.ToLower().Contains("/spsettaskitembyid?"))
                            {
                                QueryParams = JArray.Parse(ClsGeneral.GetJsonStringFromQueryString(query3.ToString().ToLower()));
